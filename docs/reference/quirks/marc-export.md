...
items. `varFields` likewise has no `907` and no `998`. This is the same split as above and for the
same reason: `945` is generated by the *export job* from attached items, so it is not part of the bib
record any JSON surface reads. `marc` and `varFields` are two shapes of the same stored record;
capturing both is redundant.
